...
struct ConvNetImpl : public torch::nn::Module 
{
	ConvNetImpl(int64_t channels, int64_t height, int64_t width)
		...
		bn2d_64(torch::nn::BatchNorm2d(64)),
		...
		bn2d_128(torch::nn::BatchNorm2d(128)),
		...
		bn2d_256(torch::nn::BatchNorm2d(256)),
		...
		bn2d_512(torch::nn::BatchNorm2d(512)),
		...
		lin1(n, 2)
	{
		...
		register_module("bn_128", bn2d_128);
		...
		register_module("bn_256", bn2d_256);
		...
		register_module("bn_512", bn2d_512);
	...
	torch::Tensor forward(torch::Tensor x)
	{
		...
		res.index({ torch::indexing::Slice(),torch::indexing::Slice(), torch::indexing::Slice(2, res.sizes()[2] - 2), torch::indexing::Slice(2, res.sizes()[3] - 2) }) += x;
		res = torch::batch_norm(bn2d_64->forward(res), bn_64, bn_Bias_64, bn_mean_64, bn_var_64, true, 0.9, 0.001, true);
		x = res.clone();
		...
		res.index({ torch::indexing::Slice(),torch::indexing::Slice(), torch::indexing::Slice(2, res.sizes()[2] - 2), torch::indexing::Slice(2, res.sizes()[3] - 2) }) += x;
		res = torch::batch_norm(bn2d_128->forward(res), bn_128, bn_Bias_128, bn_mean_128, bn_var_128, true, 0.9, 0.001, true);
		x = res.clone();
		...
		res.index({ torch::indexing::Slice(),torch::indexing::Slice(), torch::indexing::Slice(2, res.sizes()[2] - 2), torch::indexing::Slice(2, res.sizes()[3] - 2) }) += x;
		res = torch::batch_norm(bn2d_256->forward(res), bn_256, bn_Bias_256, bn_mean_256, bn_var_256, true, 0.9, 0.001, true);
		x = res.clone();
		...
		res.index({ torch::indexing::Slice(),torch::indexing::Slice(), torch::indexing::Slice(2, res.sizes()[2] - 2), torch::indexing::Slice(2, res.sizes()[3] - 2) }) += x;
		res = torch::batch_norm(bn2d_512->forward(res), bn_512, bn_Bias_512, bn_mean_512, bn_var_512, true, 0.9, 0.001, true);
		x = res.clone();
	...
	torch::nn::BatchNorm2d bn2d_64, bn2d_128, bn2d_256, bn2d_512;

	torch::Tensor bn_64;
	torch::Tensor bn_Bias_64;
	torch::Tensor bn_mean_64;
	torch::Tensor bn_var_64;

	torch::Tensor bn_128;
	torch::Tensor bn_Bias_128;
	torch::Tensor bn_mean_128;
	torch::Tensor bn_var_128;

	torch::Tensor bn_256;
	torch::Tensor bn_Bias_256;
	torch::Tensor bn_mean_256;
	torch::Tensor bn_var_256;

	torch::Tensor bn_512;
	torch::Tensor bn_Bias_512;
	torch::Tensor bn_mean_512;
	torch::Tensor bn_var_512;
